...
module.exports.get = (model, idColumnName = 'id', userColumnName = 'user') =>  async (req, res) => {
	...
	if(entry[userColumnName] !== req.uid) return res.status(401).send('Niet toegestaan')
    }
    res.send(entry);
}
...
module.exports.findOne = (model, userColumnName = 'user') =>  async (req, res) => {
    ...
    if(userColumnName !== null && req.jwt !== undefined) conditions.where[userColumnName] = req.uid;
...
module.exports.list = (model, userColumnName = 'user') => async (req, res) => {
    const conditions = { where: {[userColumnName]: req.uid} }
...
module.exports.delete = (model, idColumnName = 'id', userColumnName = 'user') => async (req, res) => {
    const entry = await model.destroy({ where: { [idColumnName]: req.params[idColumnName]  } })
    console.log(entry);
    res.send(entry)
}
