...
export const runSalarisVerdelenScript = async (rekeningen, options = {}) => {
  ...
  for (var rekening of rekeningen){
    ...
    if(rekening['month_' + maandnummer] > 0){
      let overboeking = null;//await fetchBackend('/api/bunq/payment', {method: 'POST', body: {from: {type: 'description', value: options.from_account}, to: {type: 'description', value: rekening.rekening}, description: 'Geld apart zetten', amount: rekening['month_' + maandnummer].toString() + '.00'}});
  ...
  if(options.move_rest){
    let overboeking = null;//await fetchBackend('/api/bunq/payment', {method: 'POST', body: {from: {type: 'description', value: options.from_account}, to: {type: 'description', value: options.sparen}, description: 'Geld sparen', amount: options.sparen.toString() + '.00'}});
    console.log(overboeking); 
    //-----if(overboeking.success === false) setPreconditions({...preconditions, logging: {...preconditions.logging, [rekening.rekening]: {success: false, message: overboeking.message.Error[0].error_description}}})
  }
