...
export default class Enelogic {
    ...
    getMeasuringPoints = async () => {
        const response = await this.fetchEnelogic(this.host + '/measuringpoints?access_token=' + this.api_key).catch(
            err => {
    ...
    getData = async (datefrom, dateto, period, options) => {
        ...
        if (!['DAY', 'QUARTER_OF_AN_HOUR', 'MONTH'].includes(period))
            throw 'You have to specify period (string) as DAY or QUARTER_OF_AN_HOUR or MONTH. Given: ' + period;
